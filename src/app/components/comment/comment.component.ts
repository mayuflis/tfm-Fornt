  ...
  ngOnInit(): void {
    ...
    if (token) {
      try {
        ...
        this.isStudent =
          payload.user_role == 'student' || payload.user_role == 'Alumno';
      } catch (error) {
    ...
    this.route.params.subscribe((params) => {
      ...
      if (id) {
        //si hay un idTutor en el parametro de la url, tomarlo de la url
        this.tutorId = parseInt(id);
      } else {
        // si no: es porque esta logeado como tutor
      ...
      this.http
        .get(
          `http://localhost:3000/api/reviews/getTutorIdFromUserId/${this.tutorId}`
        )
        .subscribe(
          (data) => {
            ...
            this.http
              .get(
                `http://localhost:3000/api/reviews/isThereAReviewFromUserOfTutor/${this.userId}/${this.tutorId}`
              )
              .subscribe(
                (data) => {
            ...
            this.http
              .get(
                `http://localhost:3000/api/reviews/isThereAtLeastOneClassInCommon/${this.userId}/${this.tutorId}`
              )
              .subscribe(
                (data) => {
